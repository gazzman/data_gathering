...
def pull_ib_transactions(token, id, directory = 'InteractiveBrokers')
    ...
    fname = 'All_Transactions.xml'
    fname_ts = 'All_Transactions_%s.xml' % ts.iso8601
...
if ARGV[1]
    pull_ib_transactions(token, id, directory = ARGV[1])
else
    pull_ib_transactions(token, id)
end
